...
class publish_input_maps:
    ...
    def __init__(self):
        ...
        self.start_s = rospy.Subscriber("/amcl_pose",PoseWithCovarianceStamped,self.callbackStart,queue_size = 1)
	...
	input_map = np.array(data.data).reshape((data.info.height, data.info.width))
        unknown_locations = input_map[input_map == -1]
        input_map = 255 * input_map # Now scale by 255
        input_map = input_map.astype(np.uint8)
        input_map = np.abs(255 - input_map)
        input_map[input_map>100] = 255
        input_map[unknown_locations] = 127
	...
	if (self.curr_locX == None) or (self.curr_locY == None) or (self.goal_locX == None) or (self.goal_locY == None):
	    ...
	    x = int(round((self.current_path[i,1]-data.info.origin.position.y)/data.info.resolution))
            #print "Path x, y", x,y
            self.path_map[x, y] += 255

        dilated_path = cv2.dilate(self.path_map,(3,3),iterations = 1)


        self.path_map = cv2.resize(self.path_map, dsize=(500, 500), interpolation=cv2.INTER_CUBIC)
        input_map = cv2.resize(input_map, dsize=(500, 500), interpolation=cv2.INTER_CUBIC)
        self.path_map = cv2.resize(dilated_path, dsize=(500, 500), interpolation=cv2.INTER_CUBIC)
 	...
 	# xB = int(round((self.curr_locX-data.info.origin.position.x)/data.info.resolution))
        # yB = int(round((self.curr_locY-data.info.origin.position.y)/data.info.resolution))
