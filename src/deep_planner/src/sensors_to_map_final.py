...
class publish_input_maps:
    ...
    def callbackMap(self,data):
        ...
        the_map = cv2.resize(the_map, dsize=(the_map.shape[1]/self.down_scale, the_map.shape[0]/self.down_scale), interpolation=cv2.INTER_CUBIC)
        ...
        goselo_map, goselo_loc, theta = generate_goselo_maps(xA, yA, xB, yB, the_map, self.path_map, data.info.height/self.down_scale, data.info.width/self.down_scale)
