...
import os
import scipy
caffe_root = '/home/mohamed/caffe'  # Change this to your caffe path.
...
im_map_.fill(127)
the_map = np.zeros((m, n), dtype=np.uint8 )
the_map_pathlog = np.zeros((m, n), dtype=np.uint8 )
the_map_expand = np.zeros((m, n), dtype=np.uint8 )
...
#croping (doesn't work)
# if(xA>xB and yA >yB):
#     im_map = im_map[yB:yA+1, xB:xA+1, :]
# if(xA>xB and yB <yA):
#     im_map = im_map[yB:yA+1, xB:xA+1, :]
# if(xA<xB and yB >yA):
#     im_map = im_map[yA:yB+1, xA:xB+1, :]
# if(xA<xB and yB <yA):
#     im_map = im_map[yB:yA+1, xA:xB+1, :]
